...
class AzureScenarioSupport(BaseScenarioPlatformSupport):
  ...
  def add_commandline_parameters(cls, scenario_class, builder, defaults):
    ...
    builder.add_argument(
        '--test_azure_vnet', 
        default = defaults.get('TEST_AZURE_VNET', None),
        help='The name of the virtual net that contains the subnets')
    builder.add_argument(
        '--test_azure_subnets', 
        default = defaults.get('TEST_AZURE_SUBNETS', None),
        help='The properties (address and name) of the subnets, which is a list and elements are dicts')
    builder.add_argument(
        '--test_azure_sku', 
        default = defaults.get('TEST_AZURE_SKU', None),
        help='The properties (name, tier and capacity) of the cluster used for deploying, which is a dict')
    builder.add_argument(
        '--test_azure_baseOS', 
        default = defaults.get('TEST_AZURE_BASEOS', None),
        help='The OS version of the cluster used for deploying')
    builder.add_argument(
        '--test_azure_OSType', 
        default = defaults.get('TEST_AZURE_OSTYPE', None),
        help='The osType of the cluster used for deploying')
    builder.add_argument(
