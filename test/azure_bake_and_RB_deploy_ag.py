...
class AzureTest(st.AgentTestCase):
    ...
    def test_b_create_application_gateway(self):
        self.run_test_case(self.scenario.create_application_gateway())

    def test_c1_create_bake_and_deploy_pipeline(self):
        self.run_test_case(self.scenario.create_bake_and_deploy_pipeline(strategy='redblack'))
    ...
    def test_d_trigger_bake_and_deploy_pipeline(self):
        self.run_test_case(self.scenario.trigger_bake_and_deploy_pipeline(sg_id='v000'))
        self.run_test_case(self.scenario.trigger_bake_and_deploy_pipeline(sg_id='v001'))
        self.run_test_case(self.scenario.trigger_bake_and_RB_deploy_pipeline())

    def test_w_trigger_disable_and_destroy_pipeline(self):
        self.run_test_case(self.scenario.trigger_disable_and_destroy(sg_id='v002'))
        self.run_test_case(self.scenario.trigger_disable_and_destroy(sg_id='v001'))
...
def main():
    ...
    with open('./azure_config.json', 'r', encoding='utf-8') as f:
        ...
        f.close()
    defaults['TEST_APP'] += "p0rbag" + AzureTestScenario.DEFAULT_TEST_ID
